...
export default function SubmitDialog({ open, onClose, onSubmit }: SubmitDialogProps) {
  ...
  const canSubmit = description.trim() && liked.trim() && struggled.trim() && confidence !== null

  const handleSubmit = () => {
    if (!canSubmit) return
    onSubmit({ description, file, liked, struggled, confidence, comments })
    setDescription('')
    setFile(null)
    setLiked('')
    setStruggled('')
      ...
      <div className="bg-white rounded-16 w-[568px] max-h-[80vh] flex flex-col shadow-elevation-3 max-sm:w-[calc(100%-32px)] max-sm:max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-24 pt-24 pb-24 overflow-y-auto flex flex-col gap-16">
          ...
          <div className="flex flex-col">
            ...
            <textarea className={textareaClasses} placeholder="What clicked, surprised you, or felt genuinely useful..." value={liked} onChange={e => setLiked(e.target.value)} />
          </div>
          ...
          <div className="flex flex-col">
            <label className="cds-subtitle-md text-grey-975 mb-8">After this experience, how confident do you feel using Cursor in your daily workflow?</label>
          ...
          <div className="flex flex-col">
            <label className="cds-subtitle-md text-grey-975 mb-8">Any general comments or feedback?</label>
        ...
        <div className="px-24 pt-16 pb-24 shrink-0">
          <button
